...
class ProxyGetter:
    def __init__(self, proxy_file_name="proxy_list.txt"):
        self.proxy_list = []
        self.pathed_file_name = os.path.join(os.getcwd(), "settings", proxy_file_name)
    ...
    def reload(self):
        try:
            self.build_proxy_list()
            logger.info(f"Proxies recargados. Total disponibles: {len(self.proxy_list)}")
        except Exception as e:
            logger.error(f"Error al recargar proxies: {e}")